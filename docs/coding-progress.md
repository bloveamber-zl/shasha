...
- 标准验证路径：`./init.sh`
- 当前最高优先级未完成功能：`visual-20260428-008 接入小程序自动化视觉冒烟验收` 等待微信开发者工具服务端口开启；`cloud-20260428-002 建立云数据库与云函数基础能力` 等待真实微信云部署验收。
- 当前 blocker：微信开发者工具服务端口未开启，`npm run visual:smoke` 暂无法连接自动化端口；云开发数据库集合、权限规则和真实 openid 写入尚未在微信云环境验证。
- 最新完成变更：完成微信云开发账单读写基础代码链路，本地测试和结构校验通过；真实云端部署验收仍阻塞。
