...
- 标准验证路径：`./init.sh`
- 当前最高优先级未完成功能：`visual-20260428-008 接入小程序自动化视觉冒烟验收` 等待微信开发者工具服务端口开启；下一项业务开发建议进入 `cloud-20260428-002 建立云数据库与云函数基础能力`。
- 当前 blocker：微信开发者工具服务端口未开启，`npm run visual:smoke` 暂无法连接自动化端口；云开发数据库集合、权限规则和真实调用尚未实现。
- 最新完成变更：完成森林守护灵主题人工截图视觉验收，并修复记账页分类布局、底部遮挡、收支 tab 垂直居中问题。
